...
public class DotTreeGUI extends DrawingGUI {
	...
	public void drawTree(Graphics g, PointQuadtree<Dot> tree, int level) {
		...
		// TODO: YOUR CODE HERE
		//do the logic of drawing the levels
			g.drawOval((int) tree.getPoint().getX() - dotRadius, (int) tree.getPoint().getY() - dotRadius, dotRadius * 2, dotRadius * 2);        //draw circle around x and y of tree
			g.fillOval((int) tree.getPoint().getX() - dotRadius, (int) tree.getPoint().getY() - dotRadius, dotRadius * 2, dotRadius * 2);
			//draw lines to run through the point both ways
			g.drawLine(tree.getX2(), (int) tree.getPoint().getY(), tree.getX1(), (int) tree.getPoint().getY());
			g.drawLine((int) tree.getPoint().getX() , tree.getY2(), (int) tree.getPoint().getX() , tree.getY1());

		// Recurse with children with the next level
		// TODO: YOUR CODE HERE
		if (tree.hasChild(1)){
