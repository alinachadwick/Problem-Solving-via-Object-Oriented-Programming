...
public class QuadtreeTest {
	...
	private static void test2() {		//new test, will pass
		PointQuadtree<Dot> tree = new PointQuadtree<Dot>(new Dot(300,400, "A"), 0,0,800,600); 	// start with A
		tree.insert(new Dot(150,450, "B"));
		tree.insert(new Dot(250,550, "C"));
		tree.insert(new Dot(450,200, "D"));
		int bad = 0;
		bad += testSize(tree, 4);
		bad += testFind(tree, 0,0,900,4,4,4); 		// rect for all; circle for all; find all
		bad += testFind(tree, 150,450,10,4,3,1); 	// rect for all; circle for all; find B
		bad += testFind(tree, 250,550,10,4,3,1); 	// rect for all; circle for all; find C
		bad += testFind(tree, 450,200,50,3,2,1); 	// rect for all; circle for all; find D
		bad += testFind(tree, 250,550,150,4,3,2);	// rect for all; circle for all; find B, C
		bad += testFind(tree, 300,400,200,4,4,3);	// rect for all; circle for all; find B, C
